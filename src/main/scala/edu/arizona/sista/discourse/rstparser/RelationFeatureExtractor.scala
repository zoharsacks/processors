 ...
 * Date: 5/23/14
 */
class RelationFeatureExtractor(val filter: Set[String] = Set.empty) {
  var verbose = false

  def printFeatures(features: Counter[String]): Unit = {
  ...
  def mkFeatures(left: DiscourseTree,
    ...
    */
    ...
    if (verbose) printFeatures(features)
    features
  }
}
